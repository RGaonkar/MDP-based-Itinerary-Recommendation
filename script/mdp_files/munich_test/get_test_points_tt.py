...
def get_place_sequence(owners):
	...
	for i in owners:
		...
		if len(places_unique) > 2:              #only considering place sequences of greater than 2
			...
			# print places_unique
			test_seq.append(places_unique)     #append these place sequences to train_seq
...
all_owners = pickle.load(open("test_data.p", "rb"))
...
pickle.dump(all_place_sequence, open("tt" + "/test_seq.p", "wb"))

test_set_pairs = get_test_pairs()
# print test_set_pairs

# save the test points 
pickle.dump(test_set_pairs, open("tt" + "/test_pairs.p", "wb"))
